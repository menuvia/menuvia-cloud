> ⚠️ **ARHIVAT (2026-07-03).** Snapshot istoric — reflectă starea din mai 2026
> (migrațiile 030–035). Starea curentă: `ARCHITECTURE.md` + `tests/sql/` în CI.
...
> ⚠️ SUPERSEDED de mig 275 (sept 2026): funcția era COD MORT — filtra `status='completed'`,
> valoare pe care CHECK-ul tabelei o INTERZICE (`pending/sent/success/error/cancelled`) —
> și a fost ȘTEARSĂ. NU o programa și NU-i „repara" predicatul: `success` e singura legătură
> comandă↔`bon_number` din bază ȘI jetonul de idempotență al emiterii (mig 259), iar
> `error`/`cancelled` sunt ținta retry-ului + markerul POSIBIL DUPLICAT (mig 270).
> `pending_receipts` nu are și nu primește ștergere automată (trigger
> `trg_pending_receipts_block_delete`). Snippet-ul de mai jos e păstrat DOAR ca istoric.
...
- ✅ Configurare cron jobs pentru `bridge_mark_stale_as_error` (5 min) și
  `pending_receipts_cleanup_old` (săptămânal) — ⚠️ a doua e ȘTEARSĂ în mig 275 (cod mort;
  vezi nota de mai sus), iar prima rulează din mig 274 pe pg_cron, în bază.
