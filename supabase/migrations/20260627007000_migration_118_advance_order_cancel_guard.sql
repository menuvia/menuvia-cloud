-- ═══════════════════════════════════════════════════════════════
-- MIG 118 — ADV-1: cancel din 'served' fără guard
-- ═══════════════════════════════════════════════════════════════
-- Bug (ADV-1): branch-ul 'cancel' din advance_order permitea anularea
-- unei comenzi 'served' (mâncare deja livrată) fără cancel_reason. O
-- comandă servită care dispare din rapoarte fără motiv strică
-- integritatea evidenței/rapoartelor (pierdere de venit nejustificată,
-- audit trail incomplet).
--
-- Fix: pentru comenzile aflate în status 'served', cancel_reason devine
-- OBLIGATORIU. Pentru statusurile ne-served comportamentul rămâne
-- NESCHIMBAT (cancel_reason opțional).
--
-- Recreăm advance_order pornind de la versiunea CANONICĂ (mig 094,
-- 20260614000000_migration_094_critical_fixes.sql), VERBATIM, cu singura
-- adăugare a guard-ului ADV-1 în branch-ul 'cancel'. Semnătura, search_path,
-- rolurile, matricea de tranziții și grant-urile rămân identice.
