-- mig 146 — update_order_items: include extras (#2) + păstrare minimal-safe (M1)
-- ─────────────────────────────────────────────────────────────────────
-- Auditul round-4 (#2). `update_order_items` (mig 082) face delete+reinsert pe
-- order_items dar NU procesa deloc `extra_ids` și nu scria `extras_added`. Efect:
-- orice editare a unei comenzi care avea extras pierdea tăcut extras-urile ȘI
-- sub-evalua totalul (item_total fără prețul extras → orders.total scăzut).
--
-- Fix:
--   (a) dacă item-ul trimite `extra_ids` → validăm pe product_extras (product_id +
--       is_available, tenancy via produs), dedup pe input brut, includem prețul în
--       `item_total` și scriem `extras_added` cu cheile aliniate la mig 145
--       (extra_id/extra_name/price). Forward-compatible cu suportul client (#11).
--   (b) M1 (minimal-safe) — dacă item-ul NU trimite cheia `extra_ids` (clientul actual
--       nu o trimite încă), PĂSTRĂM `extras_added` existent: snapshot înainte de delete
--       și re-atașăm prima potrivire neutilizată pe `product_id` (în ordinea originală).
--       Astfel o editare fără schimbări de client nu mai pierde extras + nu mai sub-evaluă.
--   (c) `extra_ids` prezent dar listă goală → extras golite explicit (intenție de client).
--
-- Totalul rămâne corect fără dublă-numărare: `_refresh_order_totals` (mig 031) recalculează
-- orders.total = sum(item_total), iar extras intră o singură dată în item_total.
--
-- Recreare din corpul EFECTIV curent (mig 082), VERBATIM, cu adăugarea extras de mai sus.
-- Semnătura (uuid, jsonb, numeric), search_path (public, pg_temp), grant authenticated
-- rămân identice.
-- ─────────────────────────────────────────────────────────────────────
...
-- ── Asserție fail-closed: extras procesate în funcție ────────────────────────
do $$
declare v_src text;
begin
  ...
  if v_src is null then raise exception 'mig 146: update_order_items lipsește'; end if;
  if position('product_extras' in v_src) = 0 then
    raise exception 'mig 146: procesarea extras lipsește'; end if;
  if position('v_preserved' in v_src) = 0 then
    raise exception 'mig 146: păstrarea extras (M1) lipsește'; end if;
  raise notice 'mig 146: update_order_items extras (#2) + păstrare M1 OK';
end $$;
