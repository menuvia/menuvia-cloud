      ...
      {showCart && (
        ...
        >
          ...
          >
            ...
            {/* Checkout suggestion — products from missing categories. Gated by restaurant setting */}
            {(ctx?.restaurant.checkout_suggestion_settings?.enabled ?? false) &&
              (() => {
                const settings = ctx?.restaurant.checkout_suggestion_settings
                const maxSugg = settings?.max_suggestions ?? 2
                const customMsg =
                  settings?.message ?? '🍰 Înainte să trimiți... ai vrea ceva în plus?'
                ...
                // Suggest TOP products from categories not yet in cart, max 2
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {suggestions.map((s) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() => {
                              const hasRequired = (s.modifier_groups ?? []).some(
                                (g) => g.is_required,
                              )
                              if (hasRequired) {
                                setShowCart(false)
