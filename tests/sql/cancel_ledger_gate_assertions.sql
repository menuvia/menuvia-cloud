...
--        e neschimbată).
--   CL5  Clichet structural VIU: trigger BEFORE UPDATE ROW pe orders, funcția
--        citește registrul, `advance_order` poartă hint-ul + TOATE invariantele
--        lanțului 243/262/263/264.
--
...
begin
  ...
  if (v_tgtype & 2) = 0 or (v_tgtype & 16) = 0 or (v_tgtype & 1) = 0 then
    raise exception 'CL5 FAIL: trigger-ul trebuie să fie BEFORE UPDATE FOR EACH ROW (tgtype=%)', v_tgtype; end if;
  ...
  raise notice 'CL5 OK: trigger BEFORE UPDATE ROW + registru + invariantele advance_order';
end $$;
