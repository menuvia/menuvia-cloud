...
-- =============================================================================
-- Asserții RLS pentru datele sensibile ale afiliatului (mig 103 + 104): citirea
-- e STRICT own-only. Regresie pentru leak-urile găsite la review: un afiliat-
-- părinte NU trebuie să vadă IBAN/CUI-ul, plățile, atribuirile (stripe_customer_id,
-- referred_profile_id) sau ledger-ul (venitul) sub-afiliaților. Rulează ca rol
-- `authenticated` (RLS se aplică doar non-superuser). Self-contained, ROLLBACK.
--
-- Usage:
...
--   RLS2  părintele NU vede profilul sub-afiliatului (0)  ← leak payout reparat (103)
--   RLS3  un afiliat fără legătură NU vede nimic străin (0)
--   RLS4  același own-only pe affiliate_payouts
--   RLS5  părintele NU vede atribuirile sub-afiliatului (PII) (0)  ← leak 104
--   RLS6  părintele NU vede ledger-ul (venitul) sub-afiliatului (0)  ← leak 104
-- =============================================================================
...
-- Atribuire + ledger pentru sub-afiliat (date pe care părintele NU trebuie să le vadă).
insert into auth.users (id, email) values
  ('00000000-0000-0000-0000-0000000000d3','d3@rls.test') on conflict (id) do nothing;
insert into public.affiliate_attributions (id, affiliate_id, referred_profile_id, stripe_customer_id, status) values
  ('0b000000-0000-0000-0000-0000000000b2','0a000000-0000-0000-0000-0000000000b2',
   '00000000-0000-0000-0000-0000000000d3','cus_CHILD','active');
insert into public.affiliate_ledger
  (affiliate_id, attribution_id, leg, amount_cents, hold_until, stripe_event_id) values
  ('0a000000-0000-0000-0000-0000000000b2','0b000000-0000-0000-0000-0000000000b2',
   'setup', 1000, now(), 'evt_child_rls');
...
begin
  ...
  -- RLS5: părintele NU vede atribuirile sub-afiliatului (stripe_customer_id, PII)
  select count(*) into n from public.affiliate_attributions
   where affiliate_id='0a000000-0000-0000-0000-0000000000b2';
  if n <> 0 then raise exception 'RLS5 FAIL: LEAK — parent vede atribuirile child (%)', n; end if;
  raise notice 'RLS5 OK: atribuirile sub-afiliatului ascunse de părinte';

  -- RLS6: părintele NU vede ledger-ul (venitul) sub-afiliatului
  select count(*) into n from public.affiliate_ledger
   where affiliate_id='0a000000-0000-0000-0000-0000000000b2';
  if n <> 0 then raise exception 'RLS6 FAIL: LEAK — parent vede ledger-ul child (%)', n; end if;
  raise notice 'RLS6 OK: ledger-ul sub-afiliatului ascuns de părinte';
end $$;
