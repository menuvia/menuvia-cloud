...
--   QB2  email: scadent → numărat cu vârstă; viitor / plafonat / sending → NU.
--   QB3  email LEGAT de claim_email_batch: după claim, 0.
--   QB4  sms: aceeași pereche + claim_sms_batch.
...
--        / fără canal → NU; după claim_reservation_reminders → 0.
...
begin
  ...
  insert into public.email_queue (recipient_email, template_kind, status, scheduled_for, claimed_at) values
    ('d@qb.test', 'welcome', 'sending', now() - interval '1 hour', now()); -- în lucru → NU

  v := public.get_queue_backlog();
  v_n := (v->'cron'->'email'->>'waiting')::int - v_base;
  if v_n <> 1 then
    raise exception 'QB2 FAIL: email waiting=% peste baseline (asteptat 1: doar randul scadent, sub plafon, queued)', v_n; end if;
...
begin
  ...
  if v_n <> 1 then
    raise exception 'QB4 FAIL: sms waiting=% peste baseline (asteptat 1)', v_n; end if;
...
begin
  ...
  insert into public.reservations (id, restaurant_id, customer_name, customer_phone, customer_email, party_size, starts_at, ends_at, status) values
    -- în fereastră (24h), cu email → numărată; a intrat în fereastră acum ~3h
    ('73c00000-0000-4000-8000-000000000001', '73b00000-0000-4000-8000-000000000001', 'Ana', '0212000000', 'ana@qb.test', 2,
     now() + interval '21 hours', now() + interval '22 hours', 'confirmed'),
    ...
    ('73c00000-0000-4000-8000-000000000002', '73b00000-0000-4000-8000-000000000001', 'Ion', '0212000001', 'ion@qb.test', 2,
     now() + interval '30 hours', now() + interval '31 hours', 'confirmed'),
  ...
  if v_n <> 1 then
    raise exception 'QB6 FAIL: reminders waiting=% peste baseline (asteptat 1: fereastra reminder_hours_before + canal livrabil)', v_n; end if;
  v_age := (v->'cron'->'reminders'->>'oldest_age_s')::numeric;
  if v_age < 3 * 3600 - 60 or v_age > 3 * 3600 + 60 then
    raise exception 'QB6 FAIL: oldest_age_s=% (asteptat ~3h: acum - (starts_at - 24h))', v_age; end if;
...
begin
  ...
  if (v->'bridge'->'receipts'->>'waiting')::int - v_r <> 1 or (v->'bridge'->'tickets'->>'waiting')::int - v_t <> 1 then
    raise exception 'QB7 FAIL: bonul/tichetul pending nu apar sub bridge (%)', v->'bridge'; end if;
